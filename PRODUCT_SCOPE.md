...
- Machine contract: `capabilities`, plus `--agent` JSON and `--ndjson` streaming with `--max-bytes`, `--max-items`, `--text-limit`, `--select`, `--budget` and deterministic continuation tokens. Protocol integration: `mcp` server on stdio for Model Context Protocol (MCP) clients. The discoverable `pdf_password` contract declares the file transport, 127-byte limit and fail-closed behavior for encrypted PDFs.
