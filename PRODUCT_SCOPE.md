...
- Encrypted documents through the standard security handler: common RC4 40/128-bit files and V4/R4 `AESV2` files are supported. V5/R5 `AESV3` support is partial and does not yet establish full AES-256 interoperability, `/Perms` integrity validation, encrypted object streams, per-object `/Crypt` overrides or AES-256 password normalization; those cases remain explicit gaps. A document whose user password is empty — the common permission-only case — opens directly; otherwise the correct password is supplied through the CLI's bounded `--password-file` or the library API. The secret itself never appears in process arguments, output, traces or proof bundles.
...
- Machine contract: `capabilities`, plus `--agent` JSON and `--ndjson` streaming with `--max-bytes`, `--max-items`, `--text-limit`, `--select`, `--budget` and deterministic continuation tokens. Protocol integration: `mcp` server on stdio for Model Context Protocol (MCP) clients. The discoverable `pdf_password` contract declares the file transport, distinct before/after files for `diff`, the 127-byte limit and fail-closed behavior for encrypted PDFs.
...
- The cache is opt-in and stores only the document IR. `render`, `contact-sheet`, `crop`, `bundle`, `replay`, `verify`, `diff` and `fingerprint` re-ingest the document, and `--cache-dir` is rejected for them and together with `--password-file`, so decrypted content is never persisted.
