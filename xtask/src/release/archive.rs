use super::{DOCUMENTS, EXAMPLES, TARGETS, SmokeReceipt, basename, binary, binary_names, validate_smoke_receipt};
...
fn toolchain(root: &Path) -> Result<String> {
    ...
    require(!channel.contains('-'), "INVALID_TOOLCHAIN", "Toolchain must be a pinned stable Rust release")?;
    Ok(channel.to_owned())
}
...
pub fn make_package(root: &Path, binary_path: &Path, worker_path: &Path, target: &str,
    ...
    for name in DOCUMENTS { payload.insert(name.to_owned(), contained_file(root, name)?); }
    ...
    for example in EXAMPLES { payload.insert(format!("examples/{example}"), contained_file(root, &format!("fixtures/validation/{example}"))?); }
    for schema in list_files(&root.join("schemas"), 2048)? {
        if schema.extension().is_some_and(|value| value == "json") {
            let name = schema.strip_prefix(root).map_err(corrupt)?.to_string_lossy().replace('\\', "/");
            safe_member(&name)?;
            payload.insert(name, schema);
        }
    }
    require(required_files(target)?.iter().all(|name| payload.contains_key(name)), "MISSING_SCHEMAS", "Required release resources are missing")?;
    require(payload.len() < 256, "RESOURCE_LIMIT", "Release contains too many files")?;
    ...
    require(!final_path.try_exists()? && !checksum.try_exists()?, "OUTPUT_EXISTS", "Release output already exists")?;
    ...
    for (name, path) in payload {
        ...
        require(!bytes.is_empty(), "MISSING_RELEASE_FILE", "Release resources must not be empty")?;
        let size = u64::try_from(bytes.len()).map_err(corrupt)?;
        total = total.checked_add(size).ok_or_else(|| corrupt(()))?;
        require(total <= MAX_TOTAL_BYTES - MAX_JSON_BYTES, "RELEASE_SIZE_LIMIT", "Expanded release exceeds its byte limit")?;
        let executable = name == binary_name || name == worker_name;
        if executable { binary::check_bytes(&bytes, target)?; }
        files.push(Record { path: name.clone(), size, sha256: digest(&bytes), executable });
...
fn read_entry(archive: &mut ZipArchive<File>, name: &str, maximum: u64) -> Result<Vec<u8>> {
    ...
    archive.by_name(name).map_err(corrupt)?.take(maximum + 1).read_to_end(&mut bytes).map_err(corrupt)?;
    require(u64::try_from(bytes.len()).is_ok_and(|size| size <= maximum), "RELEASE_SIZE_LIMIT", "Expanded member exceeds its byte limit")?;
    Ok(bytes)
}
...
pub fn verify_archive(path: &Path) -> Result<Manifest> {
    ...
    require(metadata.is_file() && (1..=MAX_TOTAL_BYTES).contains(&metadata.len()), "INVALID_ARCHIVE", "Archive must be a bounded regular file")?;
    let mut archive = ZipArchive::new(File::open(path)?).map_err(corrupt)?;
    require((1..=256).contains(&archive.len()), "INVALID_ARCHIVE", "Archive member count is outside its bounds")?;
    ...
    let manifests: Vec<_> = members.keys().filter(|name| name.ends_with("/release-manifest.json")).cloned().collect();
    require(manifests.len() == 1, "INVALID_MANIFEST", "Exactly one release manifest is required")?;
    let manifest: Manifest = decode(parse_json(&read_entry(&mut archive, &manifests[0], MAX_JSON_BYTES)?)?)?;
    require(manifest.schema == SCHEMA, "INVALID_MANIFEST_SCHEMA", "Release manifest schema is unsupported")?;
    ...
    for record in &manifest.files {
        ...
        require(previous.is_none_or(|name| name < record.path.as_str()), "RELEASE_CONTENT_MISMATCH", "Manifest paths must be sorted and unique")?;
        previous = Some(&record.path);
        let full = format!("{base}/{}", record.path);
        require(expected.insert(full.clone()), "INVALID_MANIFEST", "Manifest path is duplicated")?;
        let executable = record.path == binary_name || record.path == worker_name;
        require(record.executable == executable, "INVALID_EXECUTABLE_MODE", "Only the two native binaries may be executable")?;
    ...
    let declared: BTreeSet<_> = manifest.files.iter().map(|record| record.path.clone()).collect();
    require(required_files(&manifest.target)?.is_subset(&declared), "INCOMPLETE_RELEASE", "Archive lacks required binaries, documentation, licenses, examples or schemas")?;
    Ok(manifest)
}
...
pub fn extract_verified(path: &Path, destination: &Path) -> Result<(PathBuf, Manifest)> {
    ...
    let result = (|| {
        ...
        let manifest_bytes = read_entry(&mut archive, &format!("{base}/release-manifest.json"), MAX_JSON_BYTES)?;
        let extracted_manifest: Manifest = decode(parse_json(&manifest_bytes)?)?;
        require(extracted_manifest == manifest, "ARCHIVE_CHANGED", "Manifest changed after verification")?;
        write_new(&destination.join("release-manifest.json"), &manifest_bytes, false)?;
        require(sha256_file(path)? == before, "ARCHIVE_CHANGED", "Archive changed during extraction")?;
    ...
    if result.is_err() { fs::remove_dir_all(destination)?; }
    result
}
...
pub fn verify_sidecar(path: &Path) -> Result<String> {
    ...
    require(read_bytes(&sidecar, 1024)? == expected.as_bytes(), "ARCHIVE_CHECKSUM_MISMATCH", "Archive checksum differs from its sidecar")?;
    Ok(hash)
}

pub fn collect(directory: &Path) -> Result<Value> {
    let paths = flat_files(directory, "zip", 10_000)?;
    require(paths.len() == TARGETS.len(), "INCOMPLETE_RELEASE_SET", "Exactly five platform archives are required")?;
    ...
    for path in paths {
        ...
        let receipt: SmokeReceipt = decode(read_json(&directory.join(format!("smoke-{}.json", manifest.target)))?)?;
