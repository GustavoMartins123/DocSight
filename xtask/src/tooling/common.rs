        ...
        impl<'de> Visitor<'de> for UniqueVisitor {
            ...
            fn visit_map<A: MapAccess<'de>>(
                ...
                while let Some((key, UniqueValue(value))) =
                    entries.next_entry::<String, UniqueValue>()?
                {
...
pub fn parse_json(bytes: &[u8]) -> Result<Value> {
    Ok(serde_json::from_slice::<UniqueValue>(bytes)?.0)
}
...
pub fn read_bytes(path: &Path, maximum: u64) -> Result<Vec<u8>> {
    ...
    require(
        metadata.is_file(),
        "INVALID_FILE",
        "Input must be a regular file without symlinks",
    )?;
    require(
    ...
    let limit = maximum
        .checked_add(1)
        .ok_or_else(|| ToolError::new("INVALID_LIMIT", "Size limit overflows"))?;
    let mut bytes = Vec::new();
    File::open(path)?.take(limit).read_to_end(&mut bytes)?;
    require(
        u64::try_from(bytes.len()).is_ok_and(|length| length <= maximum),
        "FILE_SIZE_LIMIT",
        "Input grew beyond the size limit",
    )?;
    Ok(bytes)
}

pub fn read_json(path: &Path) -> Result<Value> {
    parse_json(&read_bytes(path, MAX_JSON_BYTES)?)
}
    ...
    for character in pretty.chars() {
        if character.is_ascii() && character != '\u{7f}' {
            ...
            for unit in character.encode_utf16(&mut units) {
                use std::fmt::Write;
                write!(ascii, "\\u{:04x}", *unit)
                    .map_err(|_| ToolError::new("JSON_ENCODING", "Cannot encode canonical JSON"))?;
            }
        }
    }
    ascii.push('\n');
...
pub fn write_new(path: &Path, bytes: &[u8], executable: bool) -> Result<()> {
    ...
    #[cfg(not(unix))]
    require(
        !executable || !bytes.is_empty(),
        "INVALID_FILE",
        "Executable output must not be empty",
    )?;
    temporary.as_file().sync_all()?;
    temporary
        .persist_noclobber(path)
        .map_err(|error| ToolError::from(error.error))?;
    Ok(())
}
...
pub fn sha256_file(path: &Path) -> Result<String> {
    ...
    require(
        metadata.is_file(),
        "INVALID_FILE",
        "Digest input must be a regular file",
    )?;
    require(
        metadata.len() <= 536_870_912,
        "FILE_SIZE_LIMIT",
        "Digest input exceeds its byte limit",
    )?;
    ...
    loop {
        ...
        total = total
            ...
            .ok_or_else(|| ToolError::new("FILE_SIZE_LIMIT", "Digest input size overflow"))?;
        require(
            total <= 536_870_912,
            "FILE_SIZE_LIMIT",
            "Digest input grew beyond its byte limit",
        )?;
...
pub fn checked_revision(value: &str) -> Result<()> {
    require(
        is_hex(value, 40),
        "INVALID_REVISION",
        "Revision must be a full lowercase Git commit SHA",
    )
}
...
pub fn safe_member(value: &str) -> Result<&str> {
    ...
    require(
        valid,
        "UNSAFE_ARCHIVE_PATH",
        "Path must be portable, relative and contained",
    )?;
    Ok(value)
}
...
pub fn contained_file(root: &Path, relative: &str) -> Result<PathBuf> {
    ...
    require(
        selected.is_file() && selected.canonicalize()?.starts_with(&base),
        "INVALID_FILE",
        "Input must be a contained regular file",
    )?;
    Ok(selected)
}
...
pub fn read_json_limit(path: &Path, limit: u64) -> Result<Value> {
    parse_json(&read_bytes(path, limit)?)
}
pub fn decode<T: serde::de::DeserializeOwned>(value: Value) -> Result<T> {
    serde_json::from_value(value)
        .map_err(|_| ToolError::new("INVALID_FIELDS", "Input fields or field types are invalid"))
}
pub fn text(bytes: &[u8]) -> Result<&str> {
    std::str::from_utf8(bytes)
        .map_err(|_| ToolError::new("INVALID_UTF8", "Input must contain valid UTF-8"))
}
pub fn field<'a>(value: &'a Value, key: &str) -> Result<&'a Value> {
    value
        .get(key)
        .ok_or_else(|| ToolError::new("INVALID_FIELDS", "A required field is missing"))
}
pub fn string(value: &Value) -> Result<&str> {
    value
        .as_str()
        .ok_or_else(|| ToolError::new("INVALID_FIELDS", "Expected a string"))
}
pub fn array(value: &Value) -> Result<&[Value]> {
    value
        .as_array()
        .map(Vec::as_slice)
        .ok_or_else(|| ToolError::new("INVALID_FIELDS", "Expected an array"))
}
pub fn exact_keys(value: &Value, keys: &[&str]) -> Result<()> {
    require(
...
pub fn bounded_integer(value: &Value, minimum: u64, maximum: u64) -> Result<u64> {
    value
        .as_u64()
        .filter(|n| *n >= minimum && *n <= maximum)
        .ok_or_else(|| ToolError::new("INVALID_INTEGER", "Integer is outside its permitted range"))
}
pub fn finite_number(value: &Value, minimum: f64, maximum: f64) -> Result<f64> {
    value
        .as_f64()
        .filter(|n| n.is_finite() && *n >= minimum && *n <= maximum)
        .ok_or_else(|| {
            ToolError::new(
                "INVALID_NUMBER",
                "Number must be finite and within its limits",
            )
        })
}
...
pub fn checked_version(version: &str) -> Result<()> {
    ...
    require(
        valid,
        "INVALID_VERSION",
        "Version must be a canonical semantic release version",
    )
}

pub fn checked_digest(value: &str) -> Result<()> {
    require(
        is_hex(value, 64),
        "INVALID_DIGEST",
...
pub fn list_files(root: &Path, maximum: usize) -> Result<Vec<PathBuf>> {
    ...
    while let Some(directory) = pending.pop() {
        for item in fs::read_dir(directory)? {
            ...
            count = count
                .checked_add(1)
                .ok_or_else(|| ToolError::new("RESOURCE_LIMIT", "Resource count overflow"))?;
            require(
                count <= maximum,
                "RESOURCE_LIMIT",
                "Directory traversal exceeds its limit",
            )?;
            let kind = item.file_type()?;
            require(
                !kind.is_symlink(),
                "INVALID_FILE",
                "Directory resources cannot follow symlinks",
            )?;
            if kind.is_dir() {
