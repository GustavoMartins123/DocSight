...
fn emit_observation(report: &beta::Report, output: &Path) -> Result<bool> {
    emit(report, Some(output))?;
    Ok(true)
}
fn output_text(path: &Path, content: &str) -> Result<()> {
    if let Some(parent) = path
        .parent()
        .filter(|parent| !parent.as_os_str().is_empty())
    {
    ...
    write_new(path, content.as_bytes(), false)
}
    ...
    match cli.command {
        Command::Release { command } => match command {
            ...
            ReleaseCommand::Version { tag } => {
                if let Some(tag) = tag {
                    require(
                        tag == format!("v{}", workspace_version()),
                        "TAG_VERSION_MISMATCH",
                        "Selected tag must match the workspace version",
                    )?;
                }
                emit(&json!({"version": workspace_version()}), None)?;
            }
            ReleaseCommand::Package {
                ...
                let archive = release::archive::make_package(
                    &root, &binary, &worker, &target, &revision, &notices, &out,
                )?;
                emit(
                    &json!({"archive": archive.file_name().and_then(|name| name.to_str()), "sha256": sha256_file(&archive)?}),
                    None,
                )?;
            }
        ...
        Command::Notices { metadata, out } => {
            let content = release::notices::generate(
                read_json_limit(&metadata, 16_777_216)?,
                &root.join("Cargo.lock"),
            )?;
            output_text(&out, &content)?;
            emit(
                &json!({"schema": "docsight.notices-result/v1", "sha256": digest(content.as_bytes())}),
                None,
            )?;
        }
        Command::Changelog {
            ...
            let content = release::changelog::generate(
                &root,
                workspace_version(),
                &revision,
                since.as_deref().filter(|value| !value.is_empty()),
            )?;
            output_text(&out, &content)?;
            emit(
                &json!({"schema": "docsight.changelog-result/v1", "revision": revision, "sha256": digest(content.as_bytes())}),
                None,
            )?;
        }
        Command::Smoke { archive, out } => {
            let receipt = smoke::smoke_archive(&archive)?;
        ...
        Command::Beta { command } => match command {
            BetaCommand::Collect(options) => {
                let settings = beta::CollectOptions {
                    ...
                    include_document_digest: options.include_document_digest,
                };
                let report = beta::collect(&options.archive, &settings)?;
                return emit_observation(&report, &options.out);
            }
            BetaCommand::Aggregate { directory, out } => {
        ...
        Command::Readiness {
            ...
            let report = readiness::assess(&evidence, &revision, &root)?;
            emit(&report, out.as_deref())?;
            return Ok(report.ready_for_v1);
        }
        Command::RustOnly => {
    ...
    match execute(Cli::parse()) {
        ...
        Err(error) => {
            match serde_json::to_string(&error) {
                ...
                Err(_) => eprintln!(
                    "{{\"schema\":\"docsight.tooling-error/v1\",\"code\":\"SERIALIZATION_ERROR\"}}"
                ),
            }
            ExitCode::from(2)
        }
...
mod tests {
    ...
    fn collecting_a_failed_engine_outcome_still_writes_a_successful_observation() -> Result<()> {
        ...
        let report = beta::Report {
            ...
            diagnostic_codes: vec!["UNSUPPORTED_FORMAT".into()],
            unknown_diagnostic_count: 0,
        };
        assert!(emit_observation(&report, &path)?);
        let stored = read_json(&path)?;
        assert_eq!(stored["outcome"], "error");
        assert_eq!(stored["exit_code"], 10);
        assert!(emit_observation(&report, &path).is_err());
        Ok(())
    }
}
