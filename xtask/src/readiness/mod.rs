...
pub fn load_policy(root: &Path) -> Result<Policy> {
    ...
    require(
        policy.schema == "docsight.readiness-policy/v1"
            && policy
                .required_reviews
                .iter()
                .map(String::as_str)
    ...
    require(
        (5..=999).contains(&policy.minimum_beta_participants)
            && (2..=100_000).contains(&policy.minimum_corpus_documents)
            && (1..=50_000).contains(&policy.minimum_real_documents_per_format),
        "INVALID_READINESS_POLICY",
        "Readiness thresholds are outside their permitted limits",
    )?;
    Ok(policy)
}
...
pub fn identity(value: &Value, schema: &str, revision: &str, version: &str) -> Result<()> {
    require(
        ...
        "Evidence does not describe the exact candidate and schema",
    )
}
...
pub fn assess(directory: &Path, revision: &str, root: &Path) -> Result<Report> {
    ...
    let _recorded = record(
        &mut criteria,
        CRITERIA[0],
        evidence::validate_workspace(directory, revision, version),
    );
    let packages = record(
        &mut criteria,
        CRITERIA[1],
        evidence::package_evidence(directory, revision, version),
    )
    .unwrap_or_default();
    let _recorded = record(
        &mut criteria,
        ...
        campaigns::beta_evidence(
            ...
            revision,
            version,
            policy.minimum_beta_participants,
        ),
    );
    let cases = record(
        &mut criteria,
