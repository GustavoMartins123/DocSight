...
Use `--agent` when a caller needs the canonical machine contract. The profile emits JSON on stdout, keeps diagnostics out of stderr on successful requests, and emits versioned structured errors on stderr. `--ndjson` selects the NDJSON stream contract and also selects the `docsight.agent/v2` error envelope for failures; successful stdout still contains only stream records.
...
The `pdf_password` capability describes the only CLI transport for a known PDF password. Pass `--password-file <path>` globally with any non-diff document operation. For `diff`, use `--password-before-file <path>` and `--password-after-file <path>` independently; the global `--password-file` and direct argv passwords are rejected. Each file contains one byte-string line of at most 127 bytes; one trailing LF or CRLF is removed. Empty, multiline and oversized files are rejected before document parsing. The path may appear in process arguments, but the secret does not, and the secret is cleared from CLI memory after the request. Traces and proof bundles never contain it, so replaying or verifying encrypted source bytes requires the same file transport. Capabilities publish the supported RC4/AES algorithms, the explicit AES-256 partial-support limitations and exit code 12 for an unknown or incorrect password. Password guessing and recovery remain outside the protocol.

The `cache` capability describes the opt-in document IR cache. Pass `--cache-dir <path>` globally with a command listed in `applies_to_commands`; any other command, `--cache-max-bytes` or `--cache-max-entries` without `--cache-dir`, a cache path that is not a directory, and a combination with any password file fail with `USAGE`. The key combines the fields listed in `key_components`: the document SHA-256 and format, the SHA-256 of the running executable, the engine version, the IR schema version, the layout profile and the layout font fingerprint. Command options are not part of the key because every listed command derives its result from the same IR after loading it. An entry is validated in full before it is written, including its canonical form, and records whether it was produced in process or by a sandboxed worker. It is written to a temporary file and published without overwriting. Every read verifies the header schema, the exact key, the payload length and digest, the document identity and the IR version; the canonical form is verified again by `cache verify`, by `cache prune` and for any entry a sandboxed worker returns, which is the only entry a less trusted process produced. An entry that fails a check is moved to the quarantine directory with its reason, and only when the file still holds the rejected bytes, so an entry another process published in the meantime is never quarantined by mistake; the document is then parsed again. Outside `--agent` and `--quiet` the event is reported on stderr as `CACHE_ENTRY_QUARANTINED`. Cache I/O failures are not bypassed and surface as typed errors. Stdout is byte-identical on a miss, a hit and a run without the cache. With `--sandbox`, the parent hashes the document and executable, looks up the entry, and grants the worker only a private handoff directory holding the key and, on a hit, the validated entry; on a miss the worker writes its IR there and the parent validates it before committing it and discards it with `CACHE_RESULT_DISCARDED` if the executable changed during the run. When the worker rejects the entry it was given, the parent validates that entry itself before quarantining it, so a worker cannot have an entry discarded on its word alone. A cache key covers the document bytes, so an entry can only ever be served for the exact document it was produced from; keep separate cache directories for sources with different levels of trust. `cache stats|verify|prune|clear --cache-dir <path>` returns a `cache-result.json` report with entry, quarantine and temporary-file counts, never document content or paths; it runs without `--sandbox` and without machine-output limits. `verify` quarantines invalid entries, `prune` removes quarantined files, stale temporary files and entries written by another executable before enforcing limits, and `clear` removes every entry.

Isolation is implemented per platform with the same declared controls. The worker may consume 30 seconds of CPU time, enforced by the operating system on the worker process, and a separate 120-second wall-clock deadline stops a worker that is blocked rather than computing, so a busy host does not turn waiting into a CPU limit failure. Both limits fail with `BACKEND_FAILURE` and a message naming the limit that was reached. Linux confines the worker with `setrlimit`, a seccomp network filter and a Landlock filesystem ruleset. macOS confines it with `setrlimit` and a deny-by-default Seatbelt profile that denies every network operation. Windows runs it inside a per-run AppContainer without capabilities, which blocks network access through the platform filtering engine, plus a job object that caps process memory, CPU time and process count. On Windows the declared read and write paths receive a temporary access control entry for that AppContainer identity, revoked when the worker exits; a path whose access control list cannot be updated fails closed with a typed `BACKEND_FAILURE`.
...
A sandboxed NDJSON command relays each complete record as it is emitted. A caller must require process exit code `0` and a final `done` record; a non-zero exit invalidates any prefix already received. Page-scoped streams emit balanced `page.begin` and `page.end` records, or neither when the page boundary cannot fit. Continuation tokens bind the output mode, command scope, document identity and semantic target; a JSON token cannot be replayed into NDJSON or into a different page/filter scope.
...
Identical objects that occur the same number of times in both documents are paired in document order, so repeated headings, paragraphs and images are neither reported as changed nor as ambiguous, and a document compared with itself has no semantic changes. When the number of identical copies changes, or candidates tie within the ambiguity margin, an `ambiguous` lineage record intentionally omits a definitive counterpart and returns the competing candidates with their own evidence and scores. Related semantic additions or removals carry that lineage record and set `authoritative` to `false`. A changed record is likewise non-authoritative when relevant diagnostics or low-confidence reconstruction evidence are present. Inspect `evidence` before treating a diff as proof of a source-level content change. NDJSON emits `diff.summary` with both document identities, optional `diff.visual` and `diff.visual.page` records, then `diff.lineage` and `diff.semantic` records. Its continuation token is bound to the NDJSON mode, both document digests, the visual mode, DPI, threshold and artifact-presence profile; reuse against any different comparison or output mode fails closed.
...
Every document result carries a deterministic document ID and SHA-256 digest. Use `warnings`, `capability_details`, and `coverage` before treating geometry or pixels as authoritative. Every command reports warnings the same way in JSON, NDJSON and human output: diagnostics with the same code, severity, message, effect and page are merged into one record whose `occurrences` field counts them, and whose `object` is kept only when every merged diagnostic names the same object. Object-level diagnostics remain available through `evidence`, `coverage` and `context` for a specific object. `source_faithful` distinguishes an available operation from an exact representation.

Output limits are explicit. `limits.truncated` describes omitted result items, while `limits.warnings_truncated` describes omitted diagnostics. A continuation token is valid only for the output mode, command, document, and semantic target or filter that produced it. `--max-document-bytes` is the effective source limit and is also enforced against embedded trace and proof-bundle source bytes; raster output is independently bounded to 75,000,000 encoded PNG bytes.
...
Agent errors use `schemas/v2/error-envelope.json` and include a stable diagnostic code and process exit code. This includes errors from `--ndjson` without an explicit `--agent`; errors never appear as successful stream records on stdout. Exit code `0` is success; non-zero codes must be handled programmatically without matching human messages.
...
The M17 adaptive projection contract is `schemas/v2/projection-selection.json`, referenced by `schemas/v2/agent-envelope.json`. Explicit `--select` projections are published as `schemas/v2/projected-result.json`; the envelope records the selected field names in `limits.selected_fields`, and capabilities advertises the projection schema.
