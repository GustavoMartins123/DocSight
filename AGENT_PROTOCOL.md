...
With `--visual`, the canonical renderer compares every page and can write one deterministic absolute-difference PNG per page. `visual.authoritative`, `visual.evidence_status`, and `visual.reason_codes` distinguish source-faithful pixels from an evidence-limited comparison of deterministic DOCSIGHT renders. Each page repeats that classification so page-local diagnostics remain visible. An added or removed page has `change_fraction: 1`, a full-page changed region, and a diff artifact when `--out-dir` is present. Evidence-limited output also carries `DIFF_VISUAL_EVIDENCE_LIMITED`; it is valid for detecting changes in the declared render profile but must not be presented as source-faithful visual proof.
