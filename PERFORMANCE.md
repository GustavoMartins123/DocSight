...
Ceilings live in [`benchmarks/ds18-operations.json`](benchmarks/ds18-operations.json) with reference `linux/x86_64/release`. The wall-clock ceilings retain the original Windows values after the Linux measurements confirmed that every operation remains below them. Stdout byte counts are exact because agent output is deterministic, except for render entries whose paths embed the scratch directory. Linux peak-memory ceilings use twice the highest `VmHWM` observed across two complete matrix runs, rounded up to whole MiB. On a host that differs from the reference, `--check` still enforces determinism, exit codes, error envelopes and stdout byte counts, and skips wall-clock and peak-memory ceilings explicitly instead of comparing incomparable measurements.

Measured baseline on 2026-09-21 (Ubuntu 24.04, Linux 6.17, Intel Core i7-13650HX, Rust 1.96.0, release): small-document operations complete in 2.5–24.4 ms, and `diff_pair` is the heaviest entry at 40.6–43.4 ms with 133 KB of JSON. The highest observed peak memory is 11,554,816 bytes for cold `inspect_medium`; the versioned ceilings range from 10 to 23 MiB.

## Cost structure (DS18)

Process startup, hashing and cache validation dominate the small-document matrix. On the Linux reference measurement, a 20 KB DOCX inspects cold in 8.1–10.0 ms and warm in 9.2–10.2 ms, while the 68 KB, 29-page specification inspects cold in 16.9–19.4 ms and warm in 10.2–10.8 ms. The cache therefore helps once parsing and layout outweigh its validation cost, but commonly costs time on tiny inputs.

Inside the engine, page-one rasterization remains the dominant measured stage. The Linux end-to-end operation matrix renders the DOCX fixture in 10.1–10.2 ms and the PDF fixture in 20.3–21.3 ms with stable PNG digests. Earlier stage measurements found pixel-linear scaling with no superlinear raster stage on the current corpus.

The Linux matrix reduces the 29-page inspect by roughly 40–47% when warm. Historical Windows measurements found about 100 ms of sandbox overhead per invocation, so agent workflows should still batch work into fewer, larger calls. The five DS17 task scenarios remain the interaction-depth gate for those workflows.
