...
- Add `benchmark operations`, a per-operation client-to-process matrix with cold and warm runs over small DOCX/PDF fixtures, the specification document, paired and self diffs, page-one renders and a typed-error probe. Cold iterations must be byte-identical, warm output must equal cold output, render artifacts must keep their digest, and wall, stdout-byte and peak-memory ceilings in `benchmarks/ds18-operations.json` are enforced by `--check`, with wall and peak-memory ceilings skipped explicitly off the reference host.
- Record the Linux x86_64 release baseline: small-document operations complete in 2.5–24.4 ms, `diff_pair` completes in 40.6–43.4 ms, and peak `VmHWM` ranges from 4.1 to 11.6 MB across two complete matrix runs.
- Enforce the operation matrix in CI on the canonical Linux target, including wall time, determinism, exit codes, stdout byte counts and per-operation peak-memory ceilings with 100% measured headroom rounded to MiB.
- Document the Linux cost structure: cache validation commonly costs time on tiny inputs, a warm 29-page inspect is roughly 40–47% faster, and page-one renders complete in 10.1–21.3 ms with stable PNG digests.
